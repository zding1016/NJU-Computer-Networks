...
class CachingServer(TCPServer):
    ...
    def touchItem(self, path: str):
        ...
        if path in self.cacheTable:
            ...
            self.log_info(f"Found in the Cache table.")
        if headers == None or (headers != None and self.cacheTable.expired(path)):
            response = self.requestMainServer(path)
            if response == None:
                self.log_info(f"Not Found in the Main Server.")
                yield (None, None)
                raise StopIteration  
            else:
                headers = self._filterHeaders(response.getheaders())
                self.cacheTable.setHeaders(path, headers)
                #self.log_info(f"Body is '{response.read()}'")
                while True:
                    length = response.readinto(buffer)
                    self.cacheTable.appendBody(path, buffer)
                    if length == BUFFER_SIZE:
                        self.log_info(f"Full buffer with length '{length}' sent.")
                        yield (headers, buffer[:length])
                    elif length > 0:
                        self.log_info(f"buffer not full with length '{length}' sent.")
                        yield (headers, buffer[:length])
                    else:
                        self.log_info(f"transmission finished.")
                        raise StopIteration    
        else:
...
class CachingServerHttpHandler(BaseHTTPRequestHandler):
    ...
    The response head consists of status, version and multiple headers. At
    ...
    def do_GET(self):
        ...
        self.path. 
        '''
        # TODO: implement the logic to respond to a GET.
        # Remember to leverage the methods in CachingServer.
        cnt = 0
        func = self.server.touchItem(self.path)
        while True:
            try:
                response = next(func)
                if cnt == 0:
    ...
    def do_HEAD(self):
        ...
        headers.
        '''
        # TODO: implement the logic to respond to a HEAD.
        # Similar to do_GET()
        response = next(self.server.touchItem(self.path))
        if response[0] == None:
            self.send_error(HTTPStatus.NOT_FOUND, "'File not found.'")
            self.end_headers()
        else:
